...
#? Actualizar Carreray redirigir a la lista de Carreras actualizada:
@router.post("/update/career/{career_id}")
async def update_career(request: Request, career_id: int, career_name: str = Form(...), facutie_id: str = Form(...), db: Session = Depends(get_db)):
    try:
        # Editar la Facultad
